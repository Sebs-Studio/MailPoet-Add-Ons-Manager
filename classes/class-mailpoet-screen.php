...
class MailPoet_Screen {
	 ...
	 * @return  void
	 */
	public static function get_header($token = 'mailpoet', $screen_icon = 'options-general'){
		do_action('mailpoet_screen_before', $token, $screen_icon);

		$html = '<div class="wrap mailpoet-wrap">'."\n";

		echo $html;

		do_action('mailpoet_screen_header_before_content', $token, $screen_icon);
	} // End get_header()
	...
	public static function get_footer(){
		do_action('mailpoet_screen_footer_after_content');

		$html = '</div><!--/.wrap mailpoet-wrap-->'."\n";

		echo $html;

		do_action('mailpoet_screen_after');
	} // End get_footer()
	 ...
	 */
	public static function get_navigation_tabs($menu_key = 'mailpoet'){
		$html = '';

		$screens = MailPoet_Screen::get_available_screens();
		...
		if(0 < count($screens)){
			foreach($screens as $k => $v){
				...
				$url = add_query_arg('page', 'mailpoet-add-ons', admin_url('admin.php'));
